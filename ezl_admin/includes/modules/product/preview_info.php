...
if (!defined('IS_ADMIN_FLAG')) {
  die('Illegal Access');
    ...
    if (zen_not_null($_POST)) {
      ...
      $logiciel_assoc = $_POST['logiciel_assoc'];
      $carac_technique = $_POST['carac_technique'];
      $accessoire_assoc = $_POST['accessoire_assoc'];
      $applic_temoignage = $_POST['applic_temoignage'];
      $products_url = $_POST['products_url'];
      } else {
      $product = $db->Execute("select p.products_id, pd.language_id, pd.products_name,
                                      pd.products_description, pd.avantages, pd.carac_technique,
                                      pd.logiciel_assoc, pd.accessoire_assoc, pd.applic_temoignage, pd.products_url,
                                      p.products_quantity,
    ...
    for ($i=0, $n=sizeof($languages); $i<$n; $i++) {
      if (isset($_GET['read']) && ($_GET['read'] == 'only')) {
        ...
        $pInfo->avantages = zen_db_prepare_input($avantages[$languages[$i]['id']]);
        $pInfo->logiciel_assoc = zen_db_prepare_input($logiciel_assoc[$languages[$i]['id']]);
        $pInfo->carac_technique = zen_db_prepare_input($carac_technique[$languages[$i]['id']]);
        $pInfo->accessoire_assoc = zen_db_prepare_input($accessoire_assoc[$languages[$i]['id']]);
        $pInfo->applic_temoignage = zen_db_prepare_input($applic_temoignage[$languages[$i]['id']]);
...
//auto replace with defined missing image
            ...
            if ($_POST['image_delete'] == 1 || $products_image_name == '' and PRODUCTS_IMAGE_NO_IMAGE_STATUS == '1') {
              ...
              <div id="ongletAccessAssoc" onclick="affiche_onglet(\'ongletAccessAssoc\', \'productAccesAssoc\');"><p>Accesssoires associ&eacute;s</p></div>
              ...
              <div id="descriptTextContener">
                <div  id="productDescription" style="display:block;margin-top:50px;" class="biggerText">'.$pInfo->products_description.'</div>
                <div  id="productAvantages"  style="display:none;margin-top:50px;" class="biggerText" >'.$pInfo->avantages.'</div>
                <div  id="productCaracTech"  style="display:none;margin-top:50px;" class="biggerText">'.$pInfo->logiciel_assoc.'</div>
                <div  id="productLogiAssoc"  style="display:none;margin-top:50px;" class="biggerText">'.$pInfo->carac_technique.'</div>
                <div  id="productAccesAssoc" style="display:none;margin-top:50px;" class="biggerText">'.$pInfo->accessoire_assoc.'</div>
                <div  id="productAppliAssoc" style="display:none;margin-top:50px;" class="biggerText">'.$pInfo->applic_temoignage.'</div>
              </div>
            </div>';
            } else {
              ...
              <div id="ongletAppliAssoc" onclick="affiche_onglet(\'ongletAppliAssoc\', \'productAppliAssoc\');"><p>Application /<br/> T&eacute;moignages</p></div>
              <div id="descriptTextContener">
                <div  id="productDescription" style="display:block" class="biggerText">'.$pInfo->products_description.'</div>
                <div  id="productAvantages"  style="display:none" class="biggerText" >'.$pInfo->avantages.'</div>
                <div  id="productCaracTech"  style="display:none" class="biggerText">'.$pInfo->logiciel_assoc.'</div>
                <div  id="productLogiAssoc"  style="display:none" class="biggerText">'.$pInfo->carac_technique.'</div>
                <div  id="productAccesAssoc" style="display:none" class="biggerText">'.$pInfo->accessoire_assoc.'</div>
                <div  id="productAppliAssoc" style="display:none" class="biggerText">'.$pInfo->applic_temoignage.'</div>
              </div>
            </div>';
            }
          ?>
...
/* Re-Post all POST'ed variables */
      ...
      for ($i=0, $n=sizeof($languages); $i<$n; $i++) {
        ...
        echo zen_draw_hidden_field('avantages[' . $languages[$i]['id'] . ']', htmlspecialchars(stripslashes($avantages[$languages[$i]['id']]), ENT_COMPAT, CHARSET, TRUE));
        echo zen_draw_hidden_field('logiciel_assoc[' . $languages[$i]['id'] . ']', htmlspecialchars(stripslashes($logiciel_assoc[$languages[$i]['id']]), ENT_COMPAT, CHARSET, TRUE));
       echo zen_draw_hidden_field('carac_technique[' . $languages[$i]['id'] . ']', htmlspecialchars(stripslashes($carac_technique[$languages[$i]['id']]), ENT_COMPAT, CHARSET, TRUE));
       echo zen_draw_hidden_field('accessoire_assoc[' . $languages[$i]['id'] . ']', htmlspecialchars(stripslashes($accessoire_assoc[$languages[$i]['id']]), ENT_COMPAT, CHARSET, TRUE));
       echo zen_draw_hidden_field('applic_temoignage[' . $languages[$i]['id'] . ']', htmlspecialchars(stripslashes($applic_temoignage[$languages[$i]['id']]), ENT_COMPAT, CHARSET, TRUE));
