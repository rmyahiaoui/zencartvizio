...
if (!defined('IS_ADMIN_FLAG')) {
  ...
  $champs_bdd = array('products_description','avantages','carac_technique','logiciel_assoc','accessoire_assoc','applic_temoignage');
  $champs_txt = array('description Generale','Avantages','Caracteristiques Techniques','Logiciels associes','Accessoires associes','Application et temoignages');
  // $avantages = array('2' => 'jjgfdlviulsxhnrlk');
    if (isset($_GET['pID']) && empty($_POST)) {
      $product = $db->Execute("select pd.products_name, pd.products_description,pd.avantages, pd.carac_technique,
                                      pd.logiciel_assoc, pd.accessoire_assoc, pd.applic_temoignage,
                                      products_url, p.products_id, p.products_quantity, p.products_model,
      ...
      $conteneurPOST['products_description'] = $_POST['products_description'];
      $conteneurPOST['avantages'] = $_POST['avantages'];
      $conteneurPOST['carac_technique'] = $_POST['carac_technique'];
      $conteneurPOST['logiciel_assoc'] = $_POST['logiciel_assoc'];
      $conteneurPOST['accessoire_assoc']  = $_POST['accessoire_assoc'];
      $conteneurPOST['applic_temoignage'] = $_POST['applic_temoignage'];
...
for ($y=0; $y < 6; $y++) {
      for ($i=0, $n=sizeof($languages); $i<$n; $i++) {
        ?>
          <tr>
            <td class="main" valign="top"><?php if ($i == 0) echo $champs_txt[$y]; ?></td>
            <td colspan="2">
              ...
              <tr>
                ...
                <td class="main" width="100%"><?php echo zen_draw_textarea_field(''.$champs_bdd[$y].'[' .$languages[$i]['id']. ']', 'soft', '100%', '30', htmlspecialchars((isset($conteneurPOST[$champs_bdd[$y]][$languages[$i]['id']])) ? stripslashes($conteneurPOST[$champs_bdd[$y]][$languages[$i]['id']]) : zen_get_products_affiche_box($champs_bdd[$y], $pInfo->products_id, $languages[$i]['id']), ENT_COMPAT, CHARSET, TRUE)); //,'id="'.'products_description' . $languages[$i]['id'] . '"'); ?></td>
              </tr>
            </table>
          </td>
         </tr>
...
?>
          ...
          <tr><td style="color:red">Modif lovine : Pour les images utilser le module sp&eacute;cial multi-images</td></tr>
         <!--  <tr>
...
?>        <tr><td class="main">Lier accessoire + vendu</td>
              <td class="main"><select name="accessSelect" style="width:300px;">
          <?php
            // $sql = "SELECT * FROM categories_description" ;
            $cat = $db->Execute("select products_id, products_name FROM  products_description WHERE products_id != ".$pInfo->products_id."");
              // $cat = str_replace("ιθ", "ee", $cat);
                  echo '<option value="">----------</option>';
            while(!$cat->EOF) {
                  echo '<option value="'.$cat->fields['products_id'] . '">'.$cat->fields['products_name'] . '</option>';
                    $cat->MoveNext();
