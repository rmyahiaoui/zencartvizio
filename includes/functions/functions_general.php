...
// Return table heading with sorting capabilities
  function zen_create_sort_heading($sortby, $colnum, $heading) {
    ...
    $sort_prefix = '';
    $sort_suffix = '';
    $id = intval($_GET['cPath']);
    if ($sortby) {
// href="' . zen_href_link($_GET['main_page'], zen_get_all_get_params(array('page', 'info', 'sort')) . 'page=1&sort=' . $colnum . ($sortby == $colnum . 'a' ? 'd' : 'a')) . '" title="' . zen_output_string(TEXT_SORT_PRODUCTS . ($sortby == $colnum . 'd' || substr($sortby, 0, 1) != $colnum ? TEXT_ASCENDINGLY : TEXT_DESCENDINGLY) . TEXT_BY . $heading) . '"
      if($colnum==2) { //Nom colonne ou Prix
        $stylePositionFleche = "margin-left:185px;";
      } else {
        $stylePositionFleche = "margin-left:4px;";
      }
      $styleBgPositition = "background-position: -65px -16px;";
      $sort_prefix = '<h4 onclick="request(\''.$colnum.'d\', \''.$id.'\');"  class="productListing-heading"><div  id="rangbylisteprod" style="'.$styleBgPositition.$stylePositionFleche.'">.</div>' ;
      $sort_suffix = (substr($sortby, 0, 1) == $colnum ? (substr($sortby, 1, 1) == 'a' ? PRODUCT_LIST_SORT_ORDER_ASCENDING : PRODUCT_LIST_SORT_ORDER_DESCENDING) : '');
       $sort_suffix = str_replace('-', '', $sort_suffix);
