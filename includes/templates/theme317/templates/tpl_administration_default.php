 ...
 */
?>
<div class="centerColumn" id="specialsListing"><h1 id="specialsListingHeading"><span class="title-left-bg"><span class="title-right-bg"><?php echo $breadcrumb->last(); ?></span></span></h1>
...
if (MAX_DISPLAY_SPECIAL_PRODUCTS > 0 ) {
  ...
  if ($num_products_count) {
    ...
    while (!$specials->EOF) {
		...
		$products_img = '<a href="' . zen_href_link(zen_get_info_page($specials->fields['products_id']), 'products_id=' . $specials->fields['products_id']) . '">' . (($specials->fields['products_image'] == '' and PRODUCTS_IMAGE_NO_IMAGE_STATUS == 0) ? '' : zen_image(DIR_WS_IMAGES . $specials->fields['products_image'], $specials->fields['products_name'], MEDIUM_IMAGE_WIDTH, MEDIUM_IMAGE_HEIGHT) . '</a>');
		
		$products_name = '<a class="name" href="' . zen_href_link(zen_get_info_page($specials->fields['products_id']), 'products_id=' . $specials->fields['products_id']) . '">' . zen_get_products_name($specials->fields['products_id']) . '</a>';
		
		$products_desc = substr(strip_tags($specials->fields['products_description']), 0, 50) . '...';
		
		$products_price = '<strong>' . zen_get_products_display_price($specials->fields['products_id']) . '</strong>';
		
		$products_butt = '<a href="' . zen_href_link(zen_get_info_page($specials->fields['products_id']), 'products_id=' . $specials->fields['products_id']) . '">' . zen_image_button(BUTTON_IMAGE_GOTO_PROD_DETAILS, BUTTON_GOTO_PROD_DETAILS_ALT) . '</a>';
	
		$img_col_w = SMALL_IMAGE_WIDTH + 15;
		
		$products_butt2 = '<a href="' . zen_href_link(FILENAME_PRODUCTS_NEW, zen_get_all_get_params(array('action')) . 'action=buy_now&products_id=' . $specials->fields["products_id"]) . '">' . zen_image_button(BUTTON_IMAGE_IN_CART, BUTTON_IN_CART_ALT) . '</a>';
	...
	if (SHOW_PRODUCT_INFO_COLUMNS_SPECIALS_PRODUCTS > 1 && $num_products_count > 1) {
		...
		if ($col > 0 && $col < SHOW_PRODUCT_INFO_COLUMNS_SPECIALS_PRODUCTS) {
			$tm_param = 'style="margin-left:5px;"';
		} else {
			$tm_param = '';
		}
	
    $list_box_contents[$row][$col] = array('params' =>'class="specialsListBoxContents '.$line.' centeredContent back margin2"' . ' ' . 'style="width:' . $col_width . '%;"',
    'text' => 
	
			'<div class="product-col product" ' . $tm_param . '>
										...
										<div class="right-bot">
											<div class="left-bot">
												<div class="prod-indent">
													...
													<div class="price">
														<b>Notre Prix:</b>' . $products_price . '
													</div>
													<div class="button">' . $products_butt2 .'&nbsp;&nbsp;&nbsp;'. $products_butt . '</div>
													<div class="clear"></div>
												</div>
											</div>
										</div>
										...
										<div class="right-bot">
											<div class="left-bot">
												<div class="prod-indent">
													...
													<div class="price">
														<b>Notre Prixe:</b>' . $products_price . '
													</div>
													<div class="button">' . $products_butt2 .'&nbsp;&nbsp;&nbsp;'. $products_butt . '</div>
													<div class="clear"></div>
												</div>
