...
if ($num_products_count > 0) {
  ...
  while (!$specials_index->EOF) {
	...
	$products_butt2 = '<a href="' . zen_href_link(FILENAME_PRODUCTS_NEW, zen_get_all_get_params(array('action')) . 'action=buy_now&products_id=' . $specials_index->fields["products_id"]) . '">' . zen_image_button(BUTTON_IMAGE_IN_CART, BUTTON_IN_CART_ALT) . '</a>';
  ...
  if ($specials_index->RecordCount() > 0) {
    $title = '<h2 class="centerBoxHeading"><span class="title-left-bg"><span class="title-right-bg">' . sprintf(TABLE_HEADING_SPECIALS_INDEX, strftime('%B')) . '</span></span></h2>';
