...
if ($num_products_count > 0) {
  ...
  while (!$new_products->EOF) {
	...
	$products_desc = substr(strip_tags($new_products->fields['products_description']), 0, 50) . '...';
  ...
  $products_butt2 = '<a href="' . zen_href_link(FILENAME_PRODUCTS_NEW, zen_get_all_get_params(array('action')) . 'action=buy_now&products_id=' . $new_products->fields["products_id"]) . '">' . zen_image_button(BUTTON_IMAGE_IN_CART, BUTTON_IN_CART_ALT) . '</a>';
  ...
  if ($new_products->RecordCount() > 0) {
    if (isset($new_products_category_id) && $new_products_category_id != 0) {
      ...
      $title = '<h2 class="centerBoxHeading"><span class="title-left-bg"><span class="title-right-bg">' . sprintf(TABLE_HEADING_NEW_PRODUCTS, strftime('%B')) . ($category_title != '' ? ' - ' . $category_title : '' ) . '</span></span></h2>';
    } else {
      $title = ' <h2 class="centerBoxHeading"><span class="title-left-bg"><span class="title-right-bg">' . sprintf(TABLE_HEADING_NEW_PRODUCTS, strftime('%B')) . '</span></span></h2>';
    }
