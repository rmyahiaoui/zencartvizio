...
if ($num_products_count > 0) {
  ...
  while (!$featured_products->EOF) {
	...
	$products_desc = substr(strip_tags($featured_products->fields['products_description']), 0, 50) . '...';
	...
	$products_butt2 = '<a href="' . zen_href_link(FILENAME_PRODUCTS_NEW, zen_get_all_get_params(array('action')) . 'action=buy_now&products_id=' . $featured_products->fields["products_id"]) . '">' . zen_image_button(BUTTON_IMAGE_IN_CART, BUTTON_IN_CART_ALT) . '</a>';
	...
	if (SHOW_PRODUCT_INFO_COLUMNS_FEATURED_PRODUCTS > 1 && $num_products_count > 1) {
		...
		if ($col > 0 && $col < SHOW_PRODUCT_INFO_COLUMNS_FEATURED_PRODUCTS) {
			$tm_param = 'style="margin-left:13px;"';
		} else {
			$tm_param = '';
		}
...
// -----------------------------------
    ...
    if (isset($new_products_category_id) && $new_products_category_id !=0) {
      ...
      $title = '
	  <h2 class="centerBoxHeading"><span class="title-left-bg"><span class="title-right-bg">' . TABLE_HEADING_FEATURED_PRODUCTS . ($category_title != '' ? ' - ' . $category_title : '') . '</span></span></h2>
	  ';
    } else {
      $title = '
	  <h2 class="centerBoxHeading"><span class="title-left-bg"><span class="title-right-bg">' . TABLE_HEADING_FEATURED_PRODUCTS . '</span></span></h2>
	  ';
    }
